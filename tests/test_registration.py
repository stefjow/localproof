...
def add_device(client, **overrides):
    body = {'pubkey': None, 'lat': DEV_LAT, 'lng': DEV_LNG, 'max_validations': 1}
...
def test_register_and_validate_with_browser_style_key(logged_in):
    ...
    resp = logged_in.get(f'/v2/{device_id}/{p64}/{s64}')
...
def test_duplicate_pubkey_rejected(logged_in):
    key = ECC.generate(curve='P-256')
